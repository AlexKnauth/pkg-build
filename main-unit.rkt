...
(define (make-vm
  ...
  (unless (complete-path? dir)
    (error 'make-vm "need a complete path for #:dir"))
...
(define (build-pkgs 
         ...
         ;; `ssh`, `scp`, and whatever's needed for the vm^ unit.
         ...
         ;; VirtualBox VMs (created by `make-vm`), at least one:
         #:vms vms
  ...
  (define (install vm #:one-time? [one-time? #f])
    ...
    (stop-vm (vm-name vm))
    (restore-snapshot (vm-name vm) (vm-init-snapshot vm))

    (dynamic-wind
     (lambda () (start-vm (vm-name vm)))
     (lambda ()
    ...
    (when (exists-snapshot? (vm-name vm) (vm-installed-snapshot vm))
      (delete-snapshot (vm-name vm) (vm-installed-snapshot vm)))
    (take-snapshot (vm-name vm) (vm-installed-snapshot vm)))
  ...
  (define (check-and-install vm #:one-time? [one-time? #f])
    ...
    (cond
     [(and uuid (equal? uuid (get-snapshot-uuid (vm-name vm) (vm-installed-snapshot vm))))
      (status "VM ~a is up-to-date for ~a\n" (vm-name vm) (vm-installed-snapshot vm))]
     [else
      (install vm #:one-time? one-time?)
      (define uuid (get-snapshot-uuid (vm-name vm) (vm-installed-snapshot vm)))
      (call-with-output-file*
       "install-uuids.rktd"
       #:exists 'truncate
       (lambda (o)
  ...
  (define (build-pkgs vm pkgs #:minimal? [minimal? #f])
    ...
    (restore-snapshot (vm-name vm) (vm-installed-snapshot vm))
    (dynamic-wind
     (lambda () (start-vm (vm-name vm) #:max-vms (length vms)))
     (lambda ()
       (define rt (vm-remote vm))
       (make-sure-vm-is-ready vm rt)
       (define ok?
       ...
       ok?)
     (lambda ()
       (stop-vm (vm-name vm) #:save-state? #f))))
  ...
  (unless skip-docs?
    ...
    (dynamic-wind
     ...
     (lambda ()
       (stop-vm (vm-name vm) #:save-state? #t)))
    (untgz "all-doc.tgz")
